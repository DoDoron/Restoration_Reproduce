...
class IterableSIDDMediumDataset(IterableDataset):
    def __init__(self, sidd_medium_path, train_or_test='train', cam=None, iso=None, num_patches_per_image=2898,
        ...
        # train_or_test에 'total'을 처리하는 부분 추가
        if self.train_or_test == 'total':
            # train과 test 데이터를 모두 합쳐서 사용
            train_file_names, _ = sidd_medium_filenames_tuple(
                sidd_path=self.sidd_medium_path,
                train_or_test='train',  # train 데이터 가져오기
                cam=self.cam,
                iso=self.iso,
            ...
            test_file_names, _ = sidd_medium_filenames_tuple(
                sidd_path=self.sidd_medium_path,
                train_or_test='test',  # test 데이터 가져오기
                cam=self.cam,
                iso=self.iso,
                first_im_idx=first_im_idx,
                last_im_idx=last_im_idx,
            ...
            # train과 test 데이터를 합침
            self.file_names_tuple = train_file_names + test_file_names
        else:
            self.file_names_tuple, self.cnt_inst = sidd_medium_filenames_tuple(
                sidd_path=self.sidd_medium_path,
                train_or_test=self.train_or_test,  # 'train' 또는 'test'
                cam=self.cam,
                iso=self.iso,
                first_im_idx=first_im_idx,
                last_im_idx=last_im_idx,
