...
def print_memory_usage(message=""):
    ...
    print(f"{message} Memory usage: {memory_in_mb:.2f} MB")
...
class SIDDMediumDataset(Dataset):
    ...
    def __init__(self, sidd_medium_path, train_or_test='train', cam=None, iso=None, num_patches_per_image=2898,
        ...
        for i, file_name_tuple in enumerate(self.file_names_tuple):
            print(f"################## Available file : {i} ##################")
            print_memory_usage("Before loading a tuple")
            print()
            if self.raw:
                img1, img2, nlf0, nlf1, iso, cam = load_one_tuple_images(file_name_tuple, subtract=self.subtract_images)
            else:
                ...
                nlf0 = None
                nlf1 = None
            print("---- After loading a tuple ----")
            print_memory_usage("After loading a tuple")
            ...
            # 패치 이미지 시각화
            # 패치 이미지 시각화
            def plot_patches(patches, num_patches=5, title="Patches"):
                """랜덤으로 선택된 몇 개의 패치를 시각화하고 이미지 파일로 저장합니다."""
                fig, axes = plt.subplots(1, num_patches, figsize=(15, 5))
                fig.suptitle(title, fontsize=16)

                # 랜덤으로 패치 선택
                idxs = np.random.choice(len(patches), num_patches, replace=False)
                
                for i, idx in enumerate(idxs):
                    patch = patches[idx]
                    patch = patch.transpose((1, 2, 0))  # (C, H, W) -> (H, W, C) 형태로 변환
                    
                    # 데이터 타입을 uint8로 변환 (이미지가 0-255 범위의 픽셀 값이어야 함)
            ...
            # img1_patches와 img2_patches의 크기 출력
            print(f"img1_patches shape: {img1_patches.shape}")
            print(f"img2_patches shape: {img2_patches.shape}")

            # 임의의 첫 번째 패치의 크기 출력
            print(f"Shape of one patch in img1_patches: {img1_patches[0].shape}")
            print(f"Shape of one patch in img2_patches: {img2_patches[0].shape}")
            # img1_patches와 img2_patches 시각화 및 파일로 저장
            plot_patches(img1_patches, num_patches=5, title="Noisy_Patches_img1_patches")
            plot_patches(img2_patches, num_patches=5, title="Ground_Truth_Patches_img2_patches")
            
            # 이미지를 사용해서 필요한 패치를 추출한 후, 원본 이미지를 삭제
            del img1, img2
            print_memory_usage("After deleting img1, img2")
            ...
            print()
            print()
        # 패치 사용 완료 후 img1_patches와 img2_patches 삭제
        del img1_patches, img2_patches
        print_memory_usage("After deleting img1_patches and img2_patches after model inference")
