...
2. ``flow_srgb_synthesis.py`` assumes that you have not run ``train_noise_model.py``. If you ran ``train_noise_model.py``, the `logdir`, `sidd_path`, and `model_save_dir` in `/experiments/sidd/our_model/hps.txt` will be automatically replaced, but will not change if you have only run ``flow_srgb_synthesis.py``. If you have problems running it, please change the paths.
